...
class RISCVAsmParser : public MCTargetAsmParser {
  ...
  bool isCheri() const override {
    return getSTI().getFeatureBits()[RISCV::FeatureVendorXCheri];
  }
...
#undef GET_OPERAND_DIAGNOSTIC_TYPES
  ...
  RISCVAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
    ...
    // error message.
    ABI = RISCVABI::computeTargetABI(STI, ABIName);
...
public:
  ...
  bool isUImm12() const {
    ...
    return IsValid && ((VK == RISCV::S_None) ||
                       (VK == RISCV::S_CHERIOT_COMPARTMENT_SIZE));
  }
    ...
    if (CE) {
      ...
      if (isUInt<5>(Imm)) {
        ...
        Operands.push_back(RISCVOperand::createSpecialCapReg(
            SpecialCapReg ? RISCVSpecialCapReg::getSpecialCapRegStr(SpecialCapReg->Name) : "", S, Imm));
        return ParseStatus::Success;
      }
    }
...
bool RISCVAsmParser::parseDirectiveOption() {
  ...
  if (Option == "capmode") {
    ...
    if (!getSTI().hasFeature(RISCV::FeatureVendorXCheri))
      return Error(Parser.getTok().getLoc(),
  ...
  Warning(Parser.getTok().getLoc(),
          ...
          "'exact', 'noexact', 'capmode' or 'nocapmode'");
